                            ...
                            cl_command_queue command_queue, int device)
{
    ...
    if(device == 2)
    {
        *input_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
        ...
        if(ret != CL_SUCCESS)
        {
            printf("Error in creating OpenCL buffer for array, Error code: %d\n", ret);
        }
    }
    else{
        *input_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
        ...
        if(ret != CL_SUCCESS)
        {
            printf("Error in writing array to OpenCL buffer on GPU, Error code: %d\n", ret);
        }
    }
}
                            ...
                            cl_command_queue command_queue, int device)
{
    ...
    if(device == 2){
        *input_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
        ...
        if(ret != CL_SUCCESS)
        {
            printf("Error in creating OpenCL buffer for dsize, Error code: %d\n", ret);
        }
        ...
        *input_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
        ...
        if(ret != CL_SUCCESS)
        {
            printf("Error in writing dsize to OpenCL buffer on GPU, Error code: %d\n", ret);
        }
...
void gal_cl_copy_from_device(gal_data_t *out, cl_mem *output_mem_obj, cl_command_queue command_queue)
{
    ...
    if(ret != CL_SUCCESS)
    {
        printf("Error in reading array to OpenCL buffer from GPU, Error code: %d\n", ret);
    }
}
